...
#
# Reads:   paper/barg/fits/*.rds  (nothing is refitted)
# Writes:  paper/barg/figures/*.png at 300 dpi on white
#
# Run from the project root:  Rscript paper/barg/barg_figures.R
# Set BARG_QUICK=1 to draw from paper/barg/fits_quick/ instead.
#
# The main figure is the two-panel display the manuscript carries as its
# Figure 9. It is defined in barg_main_figure.R, which the manuscript sources
# too, so the PNG written here and the figure the manuscript draws for itself
# come from one definition. Everything else here is supplementary.
# ==========================================================================
...
# MAIN FIGURE
# ==========================================================================
# The drawing itself lives in barg_main_figure.R, which paper/manuscript.qmd
# sources as well: the report's PNG and the manuscript's Figure 9 are then one
# definition drawn twice, and cannot drift apart.
source(here("paper", "barg", "barg_main_figure.R"))

save_fig("barg_main.png", barg_main_figure(fit_ref, fit_prior, fit_noland),
...
ppc_panel <- function(fit, r, window = NULL, ndraws = NDRAWS,
                      type = "dens_overlay") {
  p <- brms::pp_check(fit, resp = r, ndraws = ndraws, type = type) +
...
grouped_panel <- function(r, grp) {
  brms::pp_check(fit_ref, resp = r, type = "stat_grouped", group = grp,
                 stat = "median", ndraws = if (QUICK) 200 else 1000) +
    labs(title = resp_lab[[r]], x = NULL, y = NULL) + fig_theme +
    theme(legend.position = "none", plot.title = element_text(size = 8, face = "bold"),
          axis.text = element_text(size = 5.5), strip.text = element_text(size = 5.5))
}
save_fig("ppc_group_basin.png",
         wrap_plots(lapply(resps, function(r) grouped_panel(r, "Basin")), ncol = 3), 8.5, 6.0)
save_fig("ppc_group_locality.png",
...
ppc_stat_tbl <- bind_rows(lapply(stat_defs, function(s)
  ppc_stat(fit_ref, s$r, s$f, nd = if (QUICK) 200 else 1000, label = s$lab)))
saveRDS(ppc_stat_tbl, file.path(FITDIR, "ppc_stat_tbl.rds"))
...
p_stats <- ppc_stat_tbl |>
  ...
  theme(axis.text.y = element_blank(), strip.placement = "outside",
        ...
        legend.position = "bottom", plot.caption = element_text(size = 7, colour = GREY,
                                                                hjust = 0))
save_fig("ppc_stats.png", p_stats, 8.5, 4.6)
...
save_fig("prior_posterior.png", p_pp, 7.5, 8.0)
...
save_fig("rope_curves.png", p_rope, 7.5, 3.4)
...
p_cat <- ggplot(cat_df, aes(est, Loc)) +
  ...
  fig_theme + theme(axis.text.y = element_text(size = 5),
                    ...
                    plot.caption = element_text(size = 7, colour = GREY, hjust = 0))
save_fig("locality_intercepts.png", p_cat, 9.5, 6.0)
...
p_cor <- ggplot(cm, aes(X, Y, fill = Median)) +
  ...
  theme(panel.grid = element_blank(), axis.ticks = element_blank(),
        ...
        plot.caption = element_text(size = 7, colour = GREY, hjust = 0))
save_fig("correlation_matrix.png", p_cor, 6.4, 4.4)
...
THIN <- if (QUICK) 1 else 5
sens_draws <- lapply(names(SENS), function(nm) {
  d <- if (nm == "REF") as_draws_df(fit_ref) else {
         f <- gf(SENS[[nm]]); dd <- as_draws_df(f); rm(f); gc(); dd }
  keep <- seq(1, nrow(d), by = THIN)
  icc <- bind_rows(lapply(resps, function(r)
    data.frame(Spec = nm, Response = r,
  ...
  sl <- bind_rows(lapply(seq_len(nrow(key_slopes)), function(i) {
    ...
    data.frame(Spec = nm, Response = r, Predictor = pp,
...
icc_ecdf <- bind_rows(lapply(sens_draws, `[[`, "icc")) |>
...
save_fig("sensitivity_icc.png", p_se_icc, 8.0, 4.0)

sl_ecdf <- bind_rows(lapply(sens_draws, `[[`, "sl")) |>
...
save_fig("sensitivity_slopes.png", p_se_sl, 7.5, 5.2)
