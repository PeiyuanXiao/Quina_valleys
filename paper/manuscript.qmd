...
# The Bayesian model is read from the cache barg_fits.R writes; nothing is
# sampled here. The barg scripts go into a private environment so their mod_dat,
# ROPE_SD and variables do not overwrite the objects _analysis.R defines.
...
Technological consistency does not imply the absence of within-group variation. The multilevel model shows that technological differences do occur among localities, with locality accounting for `r icc_pct("GMsize")` of the variance in overall Quina scraper size, `r icc_pct("NScar")` in total retouch scar count, and `r icc_pct("Thickness")` in thickness, the three measures whose credible intervals lie wholly above the threshold for negligible locality-level variation. These differences are therefore concentrated in blank size and the extent of reduction, both of which can vary even under a shared technological concept because blank size depends partly on the cobble selected and reduction on how long a tool was used. What the model does not show is any clear landscape structure to this variation. Basin, height above the channel, and distance to the channel do not account for a meaningful reduction in locality-level variance, and none of the `r n_slopes` estimated slopes shows a clear directional relationship. The result should nevertheless be interpreted at the resolution of the dataset: these landscape predictors vary among `r n_localities` localities rather than among the `r n_scraper` specimens, and several posterior estimates remain too close to their priors to support strong inference. Thus, the analysis provides no evidence that landscape position organises technological variation among localities at the present scale, rather than demonstrating that such organisation is absent. Across two basins, three valleys and several terrace levels, the pattern is therefore one of a shared technological concept expressed through local variation in blank size and reduction, without a clear spatial structuring by landscape position.
