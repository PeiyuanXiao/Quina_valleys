...
Taken together, the surface and excavated Quina scrapers occupy the same technological space and show comparable within-group variation, while both remain distinct from the non-Quina scrapers. They are therefore most parsimoniously understood as products of a single technological concept.
...
A comparable case of recurrence is documented outside East Asia. Quina retouch occurs in the Levantine Acheulo-Yabrudian, but the associated blank production is largely absent, suggesting that it represents Quina retouch rather than the complete Quina system defined above [@agamInterpretingQuinaDemiQuina2020; @zaidnerEndLowerPaleolithic2016]. We therefore do not regard it as another instance of that system. Nevertheless, it is relevant to questions of origins: although West Asia lies between the two regions considered here, the Levantine occurrence predates the European Quina system by more than 100,000 years and does not continue into the subsequent Levantine Middle Palaeolithic. It thus provides no plausible chronological bridge between them, and instead illustrates that similar Quina retouch can recur without a demonstrated connection.
...
All data and code needed to reproduce the results reported here are openly available, with no restrictions on access or reuse. The research compendium is archived at \[\[OSF DOI\]\] under a CC BY 4.0 licence. The deposit holds the raw measurement records for the surface-collected Quina scrapers and resharpening flakes, the excavated Longtan retouched tools used for comparison, the river gravel survey and the locality table, together with the code that computes every number, table and figure in this paper and in Supplementary material 1 and 2.
