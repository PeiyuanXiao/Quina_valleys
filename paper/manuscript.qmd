...
These results are robust to the possible contribution of surface material collected within the Longtan site area. Excluding those specimens leaves the two Quina groups indistinguishable in both centroid position (R² = `r f(sens_perm_scltq_R2, 3)`, pseudo-F = `r f(sens_perm_scltq_F, 2)`, p `r fp(sens_perm_scltq_p)`) and dispersion (F = `r f(sens_scltq_F, 2)`, p `r fp(sens_scltq_p)`), while both remain distinct from the non-Quina scrapers in centroid position (R² = `r f(min(sens_perm_nonq_R2), 3)`–`r f(max(sens_perm_nonq_R2), 3)`, pseudo-F = `r f(min(sens_perm_nonq_F), 1)`–`r f(max(sens_perm_nonq_F), 1)`, p `r fp(sens_perm_nonq_p)`) and dispersion (F = `r f(min(sens_nonq_F), 1)`–`r f(max(sens_nonq_F), 1)`, p `r fp(sens_nonq_p)`) (Table S8; Fig. S4). <!-- we can extend this into a more extensive sensitivity analysis that leaves one surface locality out, each one, one at a time, like a jackknife, to get a distribution of R², pseudo-F, p values from each iteration, watching for the largest R², some nice visualisation potential -->
...
Technological variation shows little spatial structure. Across the `r n_localities` localities, the four landscape variables considered together explain little of the variation in the seven technological measures (R² = `r f(ls_full[["R2"]], 3)`, pseudo-F = `r f(ls_full[["F"]], 2)`, p `r fp(ls_full[["p"]])`; @tbl-landscape-permanova; @fig-site-pca). This explanatory power is no greater than expected by chance, the adjusted R² being negative (`r f(ls_rda_r2$adj.r.squared, 3)`), and no individual landscape variable shows a significant relationship with technological variation, either when controlling for the other variables or when considered alone (all p ≥ `r f(ls_any_p_min, 2)`; Table S16). Landscape position therefore provides no clear explanation for technological variation.
