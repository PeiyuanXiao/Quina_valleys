...
Whether landscape position had a structural influence on technology first requires that technological differences exist among localities. Our Bayesian multilevel model shows that such differences do exist. Locality-level variation accounts for 29% (95% CrI 12–52%) of the variation in Quina scraper size, 23% (5–49%) in total scar count, and 22% (6–47%) in thickness, while the remaining variables show no clear locality-level variation (@fig-landscape-model A). Thus, differences among localities are expressed mainly in the dimensions of Quina scrapers.

```{r}
#| label: fig-landscape-model
#| fig-cap: "Landscape structure of technological variation at the scale of the individual specimen, from the Bayesian multivariate multilevel model: the seven technological measures fitted together, each with the likelihood its scale calls for, with locality carried as a random intercept correlated across responses and basin, height above the channel and distance to the channel as predictors. (A) The share of the variance in each measure that lies between localities rather than within them, on the link scale of that measure. Circles are the model with the three landscape terms and triangles the intercept-only model, joined by a line; the bar is the 95% credible interval on the model with the landscape terms. Filled circles mark the measures whose interval clears the shaded band of values below 0.05, treated here as negligible. (B) The 21 landscape slopes, each divided by the standard deviation of its own response on that response's link scale, so that all seven measures share one axis and the region of practical equivalence is a single band at ±0.1. Colour gives the sign of the posterior median, blue for negative and pink for positive. The rows are in the same order in both panels. A slope whose posterior is still more than half as wide as its prior is drawn grey and hollow, the data having said too little about it for it to be read as an estimate."
#| fig-width: 7.5
#| fig-height: 3.4
#| out-width: "5.9in"
...
barg$barg_main_figure(barg$barg_fit("ref"), barg$barg_fit("prior_ref"),
                      barg$barg_fit("noland"))
```

The three landscape variables, however, do not explain these differences. Removing basin, height above the channel, and distance to the channel leaves the estimated locality-level variation essentially unchanged (median change +13%), indicating that the observed differences are not accounted for by these landscape variables. The individual coefficients lead to the same conclusion: none of the 21 effects provides clear evidence of a directional relationship, although most remain uncertain given that these predictors vary among only `r n_localities` localities rather than among individual specimens (@fig-landscape-model B; Supplementary Bayesian Report).
...
<!-- TO REVISE: the sentences from "One weak but consistent trend" to "rather than a demonstrated spatial gradient" rest on the locality-level Spearman correlations with distance to the channel (the old @tbl-distance-correlations), which was removed together with the PERMANOVA when the landscape section was rebuilt on the Bayesian model. Nothing in the Results now supports them. The Bayesian counterpart is the distance-to-channel column of @fig-landscape-model B: on the standardised axis all seven slopes are small and none has an interval clear of zero, so the trend can no longer be described as "weak but consistent" from the reported analysis. Either rewrite these sentences from the posterior for distance to channel, or drop them. -->

The technology was similarly produced wherever it occurs. The technological measurements do not vary systematically with the basin in which a locality lies, its height above the channel, or its distance from water. This is the pattern expected of a technological system maintained to a consistent standard, its concept unchanged from locality to locality. One weak but consistent trend nevertheless deserves attention: assemblages more distant from the channels maintain thicker, more reduced tools with little corresponding increase in length or width. That said, this is not a statistically supported relationship, but a weak positive trend with distance, non-significant after correction. We therefore treat this pattern as a plausible exploratory hypothesis for future landscape-scale testing rather than a demonstrated spatial gradient. If supported, this would be consistent with observations from European Quina, where the capacity for prolonged reduction reflects the use of thick, asymmetrical flakes, and more intensive reduction occurs away from abundant raw-material sources [@hiscock2009]. Across two basins, three valleys, and several terrace levels, however, the broader pattern is one of technological consistency: the same technological system was practised in similar ways and drew on raw-material resources in similar ways.
...
Our evidence supports a consistent regional entity with surface Quina scrapers that are indistinguishable from the Longtan material in the multivariate space of six technological variables, while both are distinct from the non-Quina scrapers at Longtan. Trachyte was consistently strongly selected and sandstone avoided despite differences in local gravel-bar composition, indicating that raw-material choice was governed by fracture quality and volume rather than abundance. Technological variation shows no systematic relationship with basin, distance or height above the channel. The Quina technological concept was therefore deployed consistently and provisioned from the landscape in a uniform way across the study area.
