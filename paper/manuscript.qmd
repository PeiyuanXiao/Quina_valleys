...
- Quina scrapers from 27 <!-- BM: can we get this as inline R code? -->, localities are technologically consistent with Longtan.
...
The univariate results reinforce this pattern: none of the six variables differs between the two Quina groups, whereas all six distinguish each Quina group from the non-Quina scrapers (@fig-technological-consistency; Table S5). The equivalence test further quantifies the magnitude of this similarity, indicating that the multivariate difference between the surface-collected and Longtan Quina groups is at most approximately 26.1% <!-- BM: can we get this as inline R code? --> of the difference between the Quina and non-Quina groups, even after accounting for sampling uncertainty. Thus, the difference between the two Quina groups remains well below one-third of a meaningful between-group difference, providing quantitative support for their equivalence.
