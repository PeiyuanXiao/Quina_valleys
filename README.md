...
- [:file_folder: data](data) — the source spreadsheets and CSVs, read by every script:
  ...
  - `cache/` — downloaded and DEM-derived spatial data (SRTM DEM, hillshade, channel network, admin boundaries). Regenerated by `maps/setup.R`; not version-controlled.

- [:file_folder: scripts](scripts) — all analysis code (R), grouped to match the Results:

  - [`figure1/`](scripts/figure1) — the whole cartographic / DEM pipeline behind Figure 1, in run order. `setup.R` prepares everything the panels read (the clean site table and the cached DEM, hillshade, DEM-derived channel network and administrative boundaries); `terra_map_2D.R` draws the plan site map (A), `terra_map_3D.R` the path-traced oblique block model (B), `traverse_profile_figure.R` the traverse long profile (C) and `locator_globe_figure.R` the orthographic locator inset; `fig01_export_panels.R` then re-renders A and C at their final placed sizes into `output/figures/fig01_panels/`, from which `figures/study_area.png` is assembled by hand in Illustrator. Earlier variants — a tiled basemap, a latitude-ordered elevation profile, three geology maps, an automatic patchwork composite — were deleted once superseded; the git history has them.
