...
- [:file_folder: paper](paper): the manuscript (`manuscript.qmd`), the supplementary material (`supplementary.qmd`), the bibliographies (`references.bib`, `packages.bib`), and the analyses.
  ...
  - [`barg/`](paper/barg) is the Bayesian side of the landscape analysis, the one part of the paper that `_analysis.R` does not compute. `barg_data.R` builds the specimen-level frame (165 Quina scrapers in 26 localities), `barg_priors.R` and `barg_models.R` give the seven prior and eleven model specifications, `barg_quantities.R` derives every reported quantity, and `barg_figures.R` and `barg_main_figure.R` draw the figures, Figure 9 included. `barg_report.qmd` renders the Supplementary Bayesian Report, written under the Bayesian Analysis Reporting Guidelines (Kruschke 2021). Nothing here is run by hand: [`_targets.R`](_targets.R) fits the models, and both documents read those same fits, for Figure 9 and for Tables S17 and S18, so the figure, the tables and the report cannot drift apart.
...
A cold `tar_make()` takes about an hour: 40 minutes of MCMC sampling, then the figures and the three renders. Everything after that takes minutes. Edit a prior and the eleven fits are refitted; edit a figure and none of them are; edit prose and only that document is re-rendered. There is deliberately no reduced or "quick" mode, because shortened chains would produce numbers the documents must not report.
