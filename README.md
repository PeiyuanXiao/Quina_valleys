...
4.  **Render the documents.** This is the authoritative route: both `.qmd` files source `paper/_analysis.R`, so rendering recomputes every reported number from the raw data.
    ...
    The manuscript draws Figure 9 from the MCMC fits cached in `paper/barg/fits/`, which are git-ignored and so absent from a fresh clone; run the first command of Step 6 once before this step. The chunk reads that cache and never samples, so a render cannot silently refit.
